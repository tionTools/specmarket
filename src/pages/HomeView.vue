            ...
            <section :class="{ 'pointer-events-none select-none opacity-75': isGuest }">
              ...
              <div class="mt-4 overflow-hidden rounded-xl border border-slate-300 bg-white">
                <div
                  ...
                  :key="product.id"
                  class="order-edit grid gap-2 border-b-2 border-slate-300 p-4 last:border-b-0 sm:grid-cols-[minmax(0,3.2fr)_3.75rem_4.5rem_5.5rem_4.5rem_8.75rem] sm:items-end"
                >
                  ...
                  <label class="text-xs font-medium text-slate-500"><span class="inline-flex whitespace-nowrap rounded-full bg-emerald-100 px-2 py-0.5 font-bold text-emerald-800">Себест. $</span><input :value="orderCellValue(`${order.id}-${product.id}-cost-usd`, product.costUsd ?? 0)" :readonly="editingOrderCell !== `${order.id}-${product.id}-cost-usd`" class="order-cell-edit mt-1 w-full rounded-lg border border-emerald-100 px-2 py-1.5 text-sm font-semibold text-slate-900" inputmode="decimal" type="text" @input="updateOrderNumber(product, 'costUsd', `${order.id}-${product.id}-cost-usd`, $event)" @blur="finishOrderCell(`${order.id}-${product.id}-cost-usd`)" @keydown.enter.prevent="toggleOrderCell(`${order.id}-${product.id}-cost-usd`, $event)" /></label>
                  <label class="text-xs font-medium text-slate-500"><span class="inline-flex whitespace-nowrap rounded-full bg-[#f7e2df] px-2 py-0.5 font-bold text-[#8d4d58]">Себест. ₴</span><input :value="orderCellValue(`${order.id}-${product.id}-cost`, product.cost)" :readonly="editingOrderCell !== `${order.id}-${product.id}-cost`" class="order-cell-edit mt-1 w-full rounded-lg border border-emerald-100 px-2 py-1.5 text-sm font-semibold text-slate-900" type="text" @input="updateOrderNumber(product, 'cost', `${order.id}-${product.id}-cost`, $event)" @blur="finishOrderCell(`${order.id}-${product.id}-cost`)" @keydown.enter.prevent="toggleOrderCell(`${order.id}-${product.id}-cost`, $event)" /></label>
