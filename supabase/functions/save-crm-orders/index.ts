...
type SavedOrder = {
  ...
  acquiring_percent?: number | null; extra_expenses?: number; delivery: Record<string, unknown>; items: Item[]
}
...
Deno.serve(async (request) => {
  ...
  for (const order of orders) {
    ...
    const data = {
      ...
      shipping: order.shipping, acquiring: order.acquiring, acquiring_percent: order.acquiring_percent ?? null, extra_expenses: order.extra_expenses ?? 0, delivery,
    }
    if (remoteId) {
